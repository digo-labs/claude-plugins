...
### Extract everything unless identical to defaults
Populate **all** optional fields — only omit a field if its value would be practically identical to the default preset. Specifically:
...
- **Shadows**: Extract shadow levels from the website's box-shadows. Map to the 5-level named shadow system (`ShadowConfig` with color, offsetX, offsetY, blur, spread, opacity). Steps are `xs`, `s`, `m`, `l`, `xl`.
- **Radius**: Extract the dominant border-radius value.
- **Spacing**: If spacing rhythm differs from the default `0.25rem` base, set `spacingFactor`.
- **Typography scale**: If the type scale ratio differs from default, set `typographyScaleFactor`.
