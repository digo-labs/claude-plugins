...
- `apps/design-system/src/docs.config.ts` → `DOCS_SECTIONS` (valid sections and categories)
...
- The registered MDX component inventory: the `components` map in `packages/docs/src/providers/mdx-provider.tsx` (`@digo-labs/docs` — `Callout`, `Steps`/`Step`, previews, and whatever else is registered) — this is the vocabulary the readability check works with; never assume the list, read it
...
- Every doc's frontmatter `section` + `category` pair exists in `DOCS_SECTIONS` (typos silently fall out of navigation)
...
- `src/docs.config.ts` section/category list matches the categories used in frontmatter (nav and llms.txt both derive from it)
- `llms.txt` (served at `/llms.txt`, generated by the vite docs pipeline) is complete: every doc appears in it, nothing in it points to a missing doc
