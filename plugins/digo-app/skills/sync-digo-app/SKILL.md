...
5. `sync` — the infra/config that should track the template verbatim (`scripts/**`, `eslint.config.js`, `vite*.ts`, `tsconfig*.json`, `drizzle.config.ts`, `.prettier*`, `.gitignore`, `.vscode/**`, `certs/**`, `.env.development`, `.env.example`, `src/index.css`, `src/main.tsx`).
