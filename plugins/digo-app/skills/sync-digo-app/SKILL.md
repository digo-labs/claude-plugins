...
5. `sync` — the infra/config that should track the template verbatim (`scripts/**`, `eslint.config.js`, `vite.config.ts`, `tsconfig*.json`, `.gitignore`, `.vscode/**`, `.env.development`, `.env.example`, `src/index.css`, `src/main.tsx`, `src/vite-env.d.ts`).
